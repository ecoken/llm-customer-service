...
评测集已就绪：`evaluation/golden_set.yaml`，24 道题分 6 组。
**评测脚本与实测数据尚未补齐**，本节说明的是评测怎么设计、为什么这样设计。

### 以命令生成为评测对象，而非回复文本

回复文本由模板 NLG 与 LLM 改写共同产出，同一个意思有无数种说法，逐字比对
没有意义。而命令是**结构化的、有限集合的、可精确判定的**——
`StartFlow(cancel_order)` 要么对要么错。

更关键的是命令错了后续必然全错：把「取消订单」识别成「申请售后」，
回复得再自然也是在走错误的流程、让用户白填一堆信息。因此评分分三层：

| 层级 | 判定内容 |
| --- | --- |
| 命令类型正确性 | 该 `StartFlow` 还是 `SetSlot`，还是该转人工 |
| 流程路由正确性 | `StartFlow` 启动的是不是对的那个流程 |
| 槽位抽取正确性 | 一句话里带的参数有没有被正确提取 |

### 题目构成

| 分组 | 题数 | 考察点 |
| --- | --- | --- |
| C1 流程启动 | 7 | 7 条业务流程各自能否被正确触发 |
| C2 槽位抽取 | 4 | 启动流程的同时从同一句话抽出订单号、收货人等参数 |
| C3 多轮改口 | 4 | 中途反悔、流程切换、槽位补全、重启会话 |
| C4 知识问答 | 3 | 无对应流程的规则类问题走 GraphRAG 检索 |
| C5 闲聊 | 2 | 寒暄不应触发任何业务流程 |
| C6 超范围兜底 | 4 | 越界、模糊、批量破坏性操作 |

### 刻意设计的两组易混淆对

**取消订单 vs 申请售后**——「这个订单我不想要了」与「东西坏了，我要退货」。
前者是未发货前的操作，后者是已签收后的操作，业务流程与前置条件完全不同，
走错流程会让用户白填一整套信息。

**查快递公司列表 vs 查订单物流**——「你们支持哪些快递」与「帮我查一下快递到哪了」。
字面几乎一样，意图截然不同。

### 尚未完成的部分

评测脚本需要打通 `atguigu_ai` 的命令生成接口以取出结构化命令，
并依赖完整的运行环境（MySQL `ecs` 库、Neo4j 图谱、DashScope 凭据，
以及 `atguigu-ai train` 训练好的流程检索器）。待环境就绪后补齐脚本与实测数据。
...
澄清问得太频繁，体验上和答不出来差别不大。

**GraphRAG 召回优化。** 当前是纯向量召回，补充图谱多跳关系检索后,
「这个型号的配件能不能用在那个型号上」这类需要关系推理的问题才答得了。
