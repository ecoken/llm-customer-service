...
================================================================================
# 第一步：在【短路径】下创建虚拟环境
# 注意：不要在项目目录里建 .venv！本项目路径本身已接近 120 字符，
# torch 的头文件还要再叠加约 140 字符，在未开启长路径支持的 Windows 上
# 会超出 260 字符上限，装 torch 时报 OSError: [Errno 2] No such file or directory。
python -m venv D:\venv_ecs

# 第二步：进入项目根目录（setup.py 所在的目录）
cd llm_customer_service

# 第三步：安装（选择以下任一方式，均需带上 -c constraints-atguigu.txt）
# 约束文件把 langchain 锁在 0.3.x、neo4j 驱动锁在 5.x，
# 不加会因为 langchain-community 0.4 移除了 Neo4jGraph / CypherQueryCorrector 而 ImportError。

# 方式1：开发模式安装（推荐！修改代码后立即生效，无需重新安装）
D:\venv_ecs\Scripts\python.exe -m pip install -e . -c constraints-atguigu.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

# 方式2：普通安装（代码会被复制到 site-packages，修改源码不会生效）
D:\venv_ecs\Scripts\python.exe -m pip install . -c constraints-atguigu.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

# 方式3：安装时包含开发工具（pytest、black 等）
D:\venv_ecs\Scripts\python.exe -m pip install -e ".[dev]" -c constraints-atguigu.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

# PyCharm 里把项目解释器指向 D:\venv_ecs\Scripts\python.exe
