...
def get_chat_history(tracker_state: dict[str, Any], user_id) -> str:
    ...
    兼容两种 tracker 序列化格式：
    - events 格式：{"events": [{"event": "user"/"bot", "text": ...}]}
    - atguigu_ai 的 DialogueStateTracker.to_dict() 格式：
      {"dialogue_turns": [{"user_message": {...}, "bot_messages": [{...}]}]}
    """
    chat_history = []
    ...
    if tracker_state.get("events"):
        for event in tracker_state["events"]:
        ...
        for turn in tracker_state.get("dialogue_turns") or []:
            text = ((turn.get("user_message") or {}).get("text") or "").strip()
            if text:
                chat_history.append(f"{role}:{text}")
            for bot_message in turn.get("bot_messages") or []:
...
def get_slot_value(tracker_state: dict[str, Any], slot_name: str):
    """读取槽位值

    atguigu_ai 的 slots 序列化为 {name: {"name":..., "value":...}}，
    而 events 格式下直接是 {name: value}，这里统一取出真实值。
    """
    slot = (tracker_state.get("slots") or {}).get(slot_name)
...
class GraphRAG(InformationRetrieval):
    ...
    async def search(
        ...
        user_id = None
        if tracker_state:
            user_id = get_slot_value(tracker_state, "user_id")
        # 获取聊天历史
        chat_history = get_chat_history(tracker_state, user_id) if tracker_state else query
        # 首轮对话等历史为空的情况，退化为直接使用本次提问
        if not chat_history:
            chat_history = f"user_id={user_id}:{query}" if user_id else query
        # 获取入口节点标签
...
if __name__ == "__main__":
    ...
    neo4j_url = os.getenv("NEO4J_URI", "bolt://127.0.0.1:7687")
